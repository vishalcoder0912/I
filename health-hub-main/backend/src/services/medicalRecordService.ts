...
export const medicalRecordService = {
    ...
    async createMedicalRecord(
        ...
        try {
            ...
            if (error) return handleSupabaseError(error);
            return handleSuccess(data, 'Medical record created successfully');
        } catch (error) {
            return handleSupabaseError(error);
        }
    },
    ...
    async updateMedicalRecord(recordId: string, updates: Partial<MedicalRecord>) {
        try {
            ...
            if (error) return handleSupabaseError(error);
            return handleSuccess(data, 'Medical record updated successfully');
        } catch (error) {
            return handleSupabaseError(error);
        }
    },
    ...
    async deleteMedicalRecord(recordId: string) {
        try {
            ...
            if (error) return handleSupabaseError(error);
            return handleSuccess(null, 'Medical record deleted successfully');
        } catch (error) {
            return handleSupabaseError(error);
        }
    },
