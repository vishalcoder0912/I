...
export const appointmentService = {
    ...
    async createAppointment(appointmentData: Omit<Appointment, 'id' | 'created_at' | 'updated_at'>) {
        try {
            ...
            if (error) return handleSupabaseError(error);
            return handleSuccess(data, 'Appointment created successfully');
        } catch (error) {
            return handleSupabaseError(error);
        }
    },
    ...
    async updateAppointment(appointmentId: string, updates: Partial<Appointment>) {
        try {
            ...
            if (error) return handleSupabaseError(error);
            return handleSuccess(data, 'Appointment updated successfully');
        } catch (error) {
            return handleSupabaseError(error);
        }
    },
    ...
    async cancelAppointment(appointmentId: string) {
        try {
            ...
            if (error) return handleSupabaseError(error);
            return handleSuccess(data, 'Appointment cancelled successfully');
        } catch (error) {
            return handleSupabaseError(error);
        }
    },
    ...
    async deleteAppointment(appointmentId: string) {
        try {
            ...
            if (error) return handleSupabaseError(error);
            return handleSuccess(null, 'Appointment deleted successfully');
        } catch (error) {
            return handleSupabaseError(error);
        }
    },
