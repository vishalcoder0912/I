...
export const billingService = {
  ...
  async getBillById(billId: string) {
    try {
      ...
      return { error: false, data: { ...billData, items: itemsData } };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
  ...
  async createBill(billData: Omit<Bill, 'id' | 'created_at' | 'updated_at' | 'items'>) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, data, message: 'Bill created successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
  ...
  async updateBill(billId: string, updates: Partial<Bill>) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, data, message: 'Bill updated successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
  ...
  async addBillItem(itemData: Omit<BillItem, 'id' | 'created_at'>) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, data, message: 'Bill item added successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
  ...
  async deleteBill(billId: string) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, message: 'Bill deleted successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
