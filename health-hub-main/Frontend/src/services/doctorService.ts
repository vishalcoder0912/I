...
export const doctorService = {
  ...
  async createDoctor(doctorData: Omit<Doctor, 'id' | 'created_at' | 'updated_at'>) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, data, message: 'Doctor created successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
  ...
  async updateDoctor(doctorId: string, updates: Partial<Doctor>) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, data, message: 'Doctor updated successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
  ...
  async deleteDoctor(doctorId: string) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, message: 'Doctor deleted successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
