...
export const patientService = {
  ...
  async createPatient(patientData: Omit<Patient, 'id' | 'created_at' | 'updated_at'>) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, data, message: 'Patient created successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
  ...
  async updatePatient(patientId: string, updates: Partial<Patient>) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, data, message: 'Patient updated successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
  ...
  async deletePatient(patientId: string) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, message: 'Patient deleted successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
