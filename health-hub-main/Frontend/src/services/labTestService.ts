...
import { LabTest } from '@/types/database';
...
export const labTestService = {
  ...
  async createLabTest(testData: Omit<LabTest, 'id' | 'created_at' | 'updated_at'>) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, data, message: 'Lab test created successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
  ...
  async updateLabTest(testId: string, updates: Partial<LabTest>) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, data, message: 'Lab test updated successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
  ...
  async deleteLabTest(testId: string) {
    try {
      ...
      if (error) return handleSupabaseError(error);
      return { error: false, message: 'Lab test deleted successfully' };
    } catch (error) {
      return handleSupabaseError(error);
    }
  },
